...
const TAREA_ACTION_DEFS: ReadonlyArray<Omit<ModalAction, 'visible' | 'disabled' | 'busy'>> = [
  { id: 'nuevaTarea', label: 'Nueva tarea', icon: 'bi bi-plus-lg', tone: 'primary', iconOnly: true, order: 1, title: 'Nueva tarea' },
  { id: 'borrarTarea', label: 'Borrar tarea', icon: 'bi bi-trash', tone: 'primary', iconOnly: true, order: 2, title: 'Borrar tarea del trámite' },
  ...
  { id: 'docFirmado', label: 'Doc. Firmado', icon: 'bi bi-cloud-arrow-down', tone: 'primary', order: 60, title: 'Documento Firmado' },
  { id: 'descargarDocumento', label: 'Descargar Documento', icon: 'bi bi-cloud-arrow-down', tone: 'primary', order: 70, title: 'Descargar Documento Firmado' },
  { id: 'docOriginal', label: 'Doc. Original', icon: 'bi bi-cloud-arrow-down', tone: 'primary', order: 80, title: 'Descargar Documento Original' },
  ...
  { id: 'finalizarTarea', label: 'Finalizar Tarea', icon: 'bi bi-check-circle', tone: 'primary', order: 150, title: 'Finalizar Tarea' },
]
...
export function resolveTareaHeaderActions(ctx: TareaToolbarContext): ModalAction[] {
  const visibility: Record<string, boolean> = {
    ...
    borrarTarea: !!(ctx.verAccionesdeTarea && ctx.veoBorrar && ctx.borrarconfirma),
  }
  return fromVisibilityMap(
    TAREA_ACTION_DEFS.filter((def) => def.id === 'nuevaTarea' || def.id === 'borrarTarea'),
    visibility,
  )
}
...
export function resolveTareaToolbarActions(ctx: TareaToolbarContext): ModalAction[] {
  ...
  const actions = fromVisibilityMap(
    TAREA_ACTION_DEFS.filter((def) => def.id !== 'nuevaTarea' && def.id !== 'borrarTarea'),
    visibility,
  )
