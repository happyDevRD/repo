...
export interface TareaToolbarContext {
  ...
  verxml: boolean
  veoXml: boolean
  /** Legacy: true = no se puede convertir (ocultar botón Convertir PDF). */
...
const TAREA_ACTION_DEFS: ReadonlyArray<Omit<ModalAction, 'visible' | 'disabled' | 'busy'>> = [
  ...
  { id: 'descargaXml', label: 'Modelo TEU', icon: 'bi bi-cloud-arrow-down', tone: 'secondary', order: 10, title: 'Descargar Modelo TEU' },
  { id: 'conviertePDF', label: 'Convertir a PDF', icon: 'bi bi-file-earmark-pdf', tone: 'secondary', order: 20, title: 'Convertir a PDF' },
  { id: 'metadatos', label: 'Datos ENI', icon: 'bi bi-chat-left-quote', tone: 'secondary', order: 30, title: 'Datos ENI' },
  { id: 'enviarInside', label: 'Enviar INSIDE', icon: 'bi bi-cloud-upload', tone: 'secondary', order: 40, title: 'Enviar documento a INSIDE' },
  { id: 'altaXmlDoc', label: 'Enviar INSIDE XML', icon: 'bi bi-file-earmark-code', tone: 'secondary', order: 50, title: 'Enviar INSIDE XML' },
  ...
  { id: 'firmaAtendida', label: 'Firma Atendida', icon: 'bi bi-pen', tone: 'secondary', order: 90, title: 'Firmar Digitalmente' },
  { id: 'firmaDesatendida', label: 'Firma Desatendida', icon: 'bi bi-pen', tone: 'secondary', order: 100, title: 'Firmar Digitalmente' },
  { id: 'propuestaResolucion', label: 'Prop. Resolución', icon: 'bi bi-award', tone: 'secondary', order: 110, title: 'Generar Propuesta Resolución' },
  { id: 'crearNotificacion', label: 'Crear Notificación', icon: 'bi bi-bell', tone: 'secondary', order: 120, title: 'Crear Notificación' },
  { id: 'generarSalida', label: 'Generar Salida', icon: 'bi bi-send', tone: 'secondary', order: 130, title: 'Generar Salida' },
  { id: 'tablonAnuncios', label: 'Tablón de Anuncios', icon: 'bi bi-megaphone', tone: 'secondary', order: 140, title: 'Tablón de Anuncios' },
...
function buildTareaVisibility(ctx: TareaToolbarContext): Record<string, boolean> {
  ...
  const convirtiendo = !ctx.spinnervisible
  const puedeConvertirPdf = !ctx.veoconviertePDF || convirtiendo
  const insideDisponible = !ctx.insideEnviando
  ...
  return {
    ...
    // Ocultar si no aplica (p. ej. ya es PDF); mantener visible mientras convierte
    conviertePDF: !!(acciones && archivo && puedeConvertirPdf),
    metadatos: !!(acciones && archivo && ctx.veoMetadatos),
    enviarInside: !!(acciones && archivo && insideDisponible),
    altaXmlDoc: !!(acciones && archivo && insideDisponible),
